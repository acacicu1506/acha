...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $writer = $_POST['writter']; // Genre yang dipilih
    $cast = $_POST['cast'];
    $distributor = $_POST['distributor'];
    $harga = $_POST['harga'];
    //mengupload file poster
    $target_dir_poster = "uploads/poster/"; //folder untuk menyimpan file
...
//cek ukuran file
if ($_FILES["poster"]["size"] > 500000){ //500kb
...
//cek apakah $uploadOk di set ke 0 atau kesalahan
if ($uploadOk == 0) {
    ...
    if ($_FILES["banner"]["size"] > 500000){ //500kb
    ...
    //cek apakah $uploadOkBanner sudah di set ke 0 oleh kesalahan
    if ($uploadOkBanner == 0) {
        ...
        if (move_uploaded_file($_FILES["banner"]['tmp_name'], $target_file_banner)) {
            ...
            if ($_FILES["trailer"]["size"] > 50000000){ //50 mb
                echo "Maaf, ukuran file banner terlalu besar.";
            $uploadOkTrailer = 0;
            }

            //cek apakah $uploadOkTrailer sudah di set ke 0 oleh kesalahan
    if ($uploadOkTrailer == 0) {
        echo "Maaf, file trailer tidak dapat diupload.";
    } else {
        //jika smeua cek lulus, coba untuk mengupload file
        if (move_uploaded_file($_FILES["trailer"]["tmp_name"],$target_file_trailer)) {
            //menyiapkan eksekusi query
            $stmt = $conn->prepare("INSERT INTO film (poster, trailer, banner, nama_film, judul, total_menit, usia, genre, dimensi, Producer, Director, Writer, Cast, Distributor, harga)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
